...
const shotOutcome = function (distance, angle) {
  const MAX_RANGE = [12, 15];
  const BOUNDARY_RANGE = [8, 12];
  const RUNS_RANGE = [3, 8];
  const DOT_RANGE = [0, 3];
  ...
  if (distance < 2 && Math.random() < 0.3) return WICKET;

  const isRiskArea =
    (angle < 15 || angle > 345) ||
    (angle > 80 && angle < 100) ||
    (angle > 260 && angle < 280) ||
    (distance > 10 && distance <= 12);

  if (isRiskArea && Math.random() < 0.25) return WICKET;
  if (distance >= GROUND_RADIUS) return MAXIMUM;
  if (!isRiskArea && distance > 6) return BOUNDARY;
...
const createGround = function (width, height) {
  const grid = [];
...
function buildGround(width = 25, height = 25) {
